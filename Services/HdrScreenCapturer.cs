...
internal sealed class HdrScreenCapturer
{
        ...
        IDXGIOutput6 output6, OutputDescription desc)
    {
        ...
        Helpers.CaptureLog.Write($"Output {desc.DeviceName} rect=({coords.Left},{coords.Top},{coords.Right},{coords.Bottom}) colorSpace={colorSpace}");
        ...
        {
            Helpers.CaptureLog.Write($"  DuplicateOutput1 FAILED: {ex.GetType().Name}: {ex.Message}");
        ...
        using (duplication)
        {
            ...
            for (int attempt = 0; attempt < 5 && frameResource == null; attempt++)
            {
                ...
                if (result.Failure)
                {
                    ...
                    Helpers.CaptureLog.Write($"  AcquireNextFrame FAILED: {result}");
                    return null;
                }
            }
            ...
            try
            {
                ...
                Helpers.CaptureLog.Write($"  Frame acquired. textureFormat={td.Format} {td.Width}x{td.Height}");
    ...
    private static byte[] ConvertFp16ToBgra(IntPtr src, int rowPitch, int width, int height)
    {
        ...
        Helpers.CaptureLog.Write($"  Convert: isHdr={isHdr} whitePoint={whitePoint:F3} toneMap={(isHdr ? "Reinhard(luma)" : "passthrough")}");

...
public class ScreenCaptureService : IScreenCaptureService
{
    ...
    public Bitmap CaptureRegion(int x, int y, int width, int height)
    {
        ...
        try
        {
            Helpers.CaptureLog.Session($"CaptureRegion x={x} y={y} w={width} h={height}");
            full = hdrCapturer.CaptureVirtualDesktop(out int originX, out int originY);
            Helpers.CaptureLog.Write($"DXGI path OK. full={full.Width}x{full.Height} origin=({originX},{originY})");
        ...
        {
            full?.Dispose();
            Helpers.CaptureLog.Write($"DXGI FAILED -> GDI fallback (HDR will clip bright). {ex.GetType().Name}: {ex.Message}");
